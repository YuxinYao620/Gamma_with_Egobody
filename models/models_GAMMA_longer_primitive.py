...
class GAMMAPrimitiveVAE(VAE):
    ...
    def __init__(self, configs):
        ...
        elif body_repr == 'ssm2_67_grab':
            self.in_dim = in_dim = 147*3
            self.c_dim = c_dim = 147*3
    ...
    def encode(self, x, y):
        # print("encode ,x.shape:{}".format(x.shape))
        _, hx = self.x_enc(x)
        _, hy = self.e_rnn(y)
...
class MoshRegressor(nn.Module):
    ...
    def __init__(self, config):
        ...
        # print("body_repr: {}".format(config['body_repr']))
        if config['body_repr'] == 'ssm2_67':
            self.in_dim = 67*3 # the marker dim
        elif config['body_repr'] == 'ssm2_67_grab':
            self.in_dim = 147*3
    ...
    def forward(self,
        ...
        Args:
            - marker_ref: the target markers, [b, nk, 3]#27 marker not enough, wrong with sample 
            - betas: [b,10] body shape
        ...
        ## forward pass
        # print("marker_ref.shape{}".format(marker_ref.shape))
...
class GAMMAPrimitiveCombo(VAE):
    ...
    def __init__(self, markercfg, bparamscfg):
        ...
        # mosh regressor
        # print('bparamscfg:{}'.format(bparamscfg))
    ...
    def sample_prior(self,
        ...
        print("nt {} ,nb {}".format(nt,nb))
        print("Yb_gen.shape:{}".format(Yb_gen.shape))
        return Y_gen, Yb_gen
...
class GAMMAPrimitiveVAETrainOP(TrainOP):
    ...
    def calc_loss_rollout(self, data, epoch):
        ...
        # print("n_t: {}, n_b: {}".format(n_t, n_b))
        ref_jts = ref_jts.contiguous().view(n_t, n_b, -1, 3)
        t = 0
        loss = []
        loss_info = []
        # print("ref_marker.shape:{}".format(ref_markers.shape))
        # print("nt, nb: {}, {}".format(n_t, n_b))
        while t < n_t:
            '''set time range'''
            t_lb = t
            t_ub = t+10
            ...
            ## rec loss
            # print("Y.shape:{}, Y_rec.shape:{}".format(Y.shape, Y_rec.shape))
            loss_rec = self._calc_loss_rec(Y, Y_rec)
            ## kl-divergence
            loss_kld = 0.5 * torch.mean(-1 - logvar + mu.pow(2) + logvar.exp())
            if self.lossconfig['robust_kld']:
    ...
    def train(self, batch_gen):
        ...
        if self.trainconfig['resume_training']: # can also be used for fine-tuning
            ...
            # print(self.trainconfig['save_dir'])
            if len(ckp_list)>0:
        ...
        for epoch in range(starting_epoch, self.trainconfig['num_epochs']):
            ...
            while batch_gen.has_next_rec():
                if self.max_rollout is None:
                    ...
                    optimizer.zero_grad()
                    # print("before calc_loss datashape: {}".format(data.shape))
            ...
            # print("self.traniconfig['save_dir']:{}".format(self.trainconfig['save_dir']))
            if ((1+epoch) % self.trainconfig['saving_per_X_ep']==0) :
                ...
                print(self.trainconfig['save_dir'] + "/epoch-" + str(epoch + 1) + ".ckp")
...
class GAMMARegressorTrainOP(TrainOP):
    def build_model(self):
        ...
        bm_path = '/home/yuxinyao/body_models/'
        # print('batchsize= {}'.format(self.trainconfig['batch_size']*self.modelconfig['seq_len']))
        ...
        # print('marker_path: {}'.format(marker_path))
        ...
        elif self.modelconfig['body_repr'] == 'ssm2_67_grab':
            with open(os.path.join(marker_path,'SSM2_finger_hand_palm22.json')) as f:
                markerdict = json.load(f)['markersets'][0]['indices']
            self.markers = list(markerdict.values())
        else:
            raise ValueError('other marker placement is not considered yet.')
        # print("self.markers: {}".format(self.markers))
    ...
    def calc_loss(self, x_ref, xb, betas):
        ...
        # print("body param left handpose shape: "+str(body_param['left_hand_pose'].shape))
        # print("global_orient shape: "+str( body_param['global_orient'].shape))
        # print('transl shape: '+str(body_param['transl'].shape))
        # print('body_pose shape: '+str(body_param['body_pose'].shape))
        # print('left_hand_pose shape: '+str(body_param['left_hand_pose'].shape))
        # print('right_hand_pose shape: '+str(body_param['right_hand_pose'].shape))
        # print('betas shape: '+str(body_param['betas'].shape))

        x_pred = self.bm(return_verts=True, **body_param)
        # print("self.model.markers: "+str(self.model.markers))
        x_pred = x_pred.vertices[:,self.markers,:]
        # print("self.markers.shape: "+str(len(self.markers)))
        # print("self.modelconfig['body_repr']"+str(self.modelconfig['body_repr']))
        # print("x_pred shape: "+str(x_pred.shape))
        # print("x_ref shape: "+str(x_ref.shape))
    ...
    def train(self, batch_gen):
        ...
        if self.trainconfig['resume_training']:
            ckp_list = sorted(glob.glob(os.path.join(self.trainconfig['save_dir'],'epoch-*.ckp')),
                                key=os.path.getmtime)#sort by time order 
            if len(ckp_list)>0:
                checkpoint = torch.load(ckp_list[-1])
                self.model.load_state_dict(checkpoint['model_state_dict'])
                optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        ...
        # training main loop
        print('save dir'+str(self.trainconfig['save_dir']))
        ...
        for epoch in range(starting_epoch, self.trainconfig['num_epochs']):
            ...
            while batch_gen.has_next_rec():
                ...
                # print("batch_size in train: {}".format(batch_size))
                data = batch_gen.next_batch_genderselection(batch_size,
                                                            gender)
                # print("data.shape "+str(len(data))+ ",batch_size = "+str(batch_size))
                if data is None:
                    continue
                ...
                # print("batch_betas.shape:{}".format(batch_betas.shape))
                # forward pass
                xb_new = self.model(marker_ref.detach(), batch_betas)
                # xb_new = xb_new[:,0:10]
                optimizer.zero_grad()
                # print("size of xb_new {xb}, size of batch_betas: {batch_betas}".format(xb = xb_new.shape, batch_betas= batch_betas.shape))
...
class GAMMAPrimitiveComboGenOP(TestOP):
    ...
    def generate(self,
        ...
        if not return_z:
            ...
            print("Yb_pred.shape:{}".format(Yb_pred.shape))
        else:
    ...
    def generate_primitive_to_files(self, batch_gen, n_seqs, n_gens, t_his=None):
        ...
        while idx < n_seqs:
            ...
            # print("motion_np.shape:{}".format(motion_np.shape))
            # print("batch_gen, each sequence body feature shape:{}".format(motion_np.shape))
            ...
            # generate
            # print("motion.shape:{}".format(motion.shape))
            # print("n_gen:{}".format(n_gens))
            pred_markers, pred_body_params = self.generate(motion, bparams, betas=data['betas'][0,:],
        ...
        # print("gen_results['markers'].shape:{}".format(gen_results['markers'].shape))
