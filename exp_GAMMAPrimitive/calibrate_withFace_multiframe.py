...
for record in tqdm(folder_seqs):
    ...
    for i in range(0, len(seqs),len_subseq):
        ...
        # print("jaw_pose.shape:{},eye_pose.shape:{},expression.shape:{}".format(first_frame['jaw_pose'].shape,first_frame['leye_pose'].shape,first_frame['expression'].shape))
        ...
        for j in range(0,len_subseq):
            ...
            left_hand_seq[j,:] = data['left_hand_pose']
            right_hand_seq[j,:] = data['right_hand_pose']
            jaw_pose_seq[j,:] = data['jaw_pose']
            leye_pose_seq[j,:] = data['leye_pose']
            reye_pose_seq[j,:] = data['reye_pose']
            expression_seq[j,:] = data['expression']


        data_out = {}
        if grab:
            # outfolder = "/home/yuxinyao/datasets/egobody/canicalized-camera-wearer-grab_x"+str(N_MPS)+"/" + str(record[52:])
            outfolder = "/home/yuxinyao/datasets/egobody/canicalized-camera-wearer-grab-x3/" + str(record[52:])
        else:
            outfolder = "/home/yuxinyao/datasets/egobody/canicalized-camera-wearer-Face-x"+str(N_MPS)+"/" + str(record[52:])
        ...
        # data_out['expression'] = expression_seq
        ...
        data_out['marker_ssm2_67_grab'] = markers_67
        # print("marker.shape"+str(markers_67.shape))
...
# with open("/home/yuxinyao/datasets/egobody/canicalized-camera-wearer-grab/recording_20210910_S06_S05_01/subseq_00000.pkl",'rb') as f:
...
# # print("beta: "+str(output_data['betas']))
# print(output_data.keys())

# body_param = {}
# body_param['transl'] = torch.FloatTensor(output_data['trans']).cuda()
# body_param['global_orient'] = torch.FloatTensor(output_data['global_orient']).cuda()
# body_param['betas'] = torch.FloatTensor(output_data['betas']).cuda()
# body_param['body_pose'] = torch.FloatTensor(output_data['body_pose']).cuda()
# body_param['left_hand_pose'] = torch.FloatTensor(output_data['left_hand_pose']).cuda()
# body_param['right_hand_pose'] = torch.FloatTensor(output_data['right_hand_pose']).cuda()
# body_param['jaw_pose'] = torch.FloatTensor(output_data['jaw_pose']).cuda()
# body_param['leye_pose'] = torch.FloatTensor(output_data['leye_pose']).cuda()
# body_param['reye_pose'] = torch.FloatTensor(output_data['reye_pose']).cuda()
# # body_param['expression'] = torch.FloatTensor(output_data['expression']).cuda()
...
# pcd.points = o3d.utility.Vector3dVector(output_data['marker_ssm2_67_grab'][0].reshape(-1, 3))
...
# bm = get_body_model(gender = output_data['gender'],batch_size=output_data['global_orient'].shape[0],type = 'smplx',device='cuda')

# mesh = bm(return_verts=True, **body_param)
...
# outfile_path = "/home/yuxinyao/datasets/egobody/canicalized-camera-wearer-grab/recording_20210910_S06_S05_01/subseq_00000.png"
# frame_idx = 0
